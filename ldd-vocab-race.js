/* =============================================================
   ...
   Chọn làn -> bấm LAO LÊN ĂN TỪ · chướng ngại vật mỗi 5 giây.
   ============================================================= */
(function () {
    'use strict';
    ...
    function bindEvents() {
        ...
        document.addEventListener('keydown', e => {
            if (!room || room.status !== 'playing' || !$('vocab-race-panel') || $('vocab-race-panel').style.display === 'none') return;
    ...
    async function attachRoom(id) {
        unsubscribe();
        roomChannel = raceSb.channel('race-room-v3-' + id).on('postgres_changes', { event: '*', schema: 'public', table: 'vocab_race_rooms', filter: 'id=eq.' + id }, scheduleRefresh).subscribe();
        playerChannel = raceSb.channel('race-player-v3-' + id).on('postgres_changes', { event: '*', schema: 'public', table: 'vocab_race_players', filter: 'room_id=eq.' + id }, scheduleRefresh).subscribe();
    ...
    function scheduleRefresh() {
        ...
        refreshTimer = setTimeout(() => room && refreshRoom(room.id), 50);
    }
    ...
    async function refreshRoom(id) {
        ...
        room = rr.data;
        players = pp.data || [];
        if (oldRound !== room.round_index) { claimPending = false; timeoutPending = false; charging = null; }
        render();
    }
    ...
    function leaveLocalRoom() {
        unsubscribe(); room = null; players = []; claimPending = false; timeoutPending = false; charging = null; resolvedObstacleKeys.clear();
    ...
    function renderLobby() {
        ...
        for (let slot = 1; slot <= 4; slot++) {
            ...
            const el = document.createElement('div'); el.className = 'vocab-race-roster-row' + (p && me && p.user_id === me.id ? ' is-me' : '');
    ...
    function currentQuestion() { return (Array.isArray(room.questions) ? room.questions : [])[room.round_index] || null; }
    function hideOthers() { const lr = room.last_result || {}; return !!(me && String(lr.hide_user_id || '') === String(me.id) && Number(lr.hide_round) === Number(room.round_index)); }
    ...
    function buildTrack(q) {
        ...
        for (let lane = 0; lane < 5; lane++) {
            ...
            const option = document.createElement('button'); option.type = 'button'; option.className = 'vocab-race-option'; option.textContent = (q.options || [])[lane] || '—'; option.addEventListener('click', () => chooseLane(lane));
            const road = document.createElement('div'); road.className = 'vocab-race-road';
            const warning = document.createElement('div'); warning.className = 'vocab-race-obstacle-warning'; warning.innerHTML = '<span>⚠️</span><b>CHƯỚNG NGẠI VẬT<br><em>—</em></b>';
            const obstacle = document.createElement('div'); obstacle.className = 'vocab-race-obstacle'; obstacle.innerHTML = '<span>🚧</span>';
            laneEl.append(option, road, warning, obstacle);
            players.filter(p => Number(p.lane) === lane).forEach(p => { if (!stealth || String(p.user_id) === String(me.id)) laneEl.appendChild(makeCar(p)); });
            track.appendChild(laneEl);
        }
        players.filter(p => p.lane == null).forEach(p => { if (!stealth || String(p.user_id) === String(me.id)) { const lane = track.querySelector('[data-lane="2"]'); if (lane) lane.appendChild(makeCar(p)); } });
    }
    ...
    function renderScores() {
        ...
        players.slice().sort((a,b) => b.score-a.score || a.wrong_count-b.wrong_count || Number(a.total_activation_ms)-Number(b.total_activation_ms)).forEach((p,i) => {
            const el = document.createElement('div'); el.className = 'vocab-race-mini-score' + (me && p.user_id === me.id ? ' is-me' : '') + (isEliminated(p) ? ' is-eliminated' : '');
    ...
    function renderEvent() {
        ...
        let text = '🏁 Chọn làn đúng rồi bấm LAO LÊN ĂN TỪ.'; let cls = '';
        if (lr.type === 'correct') { const p = players.find(x => String(x.user_id) === String(lr.winner_user_id)); text = '✅ ' + (p ? (p.display_name || p.email) : 'Một xe') + ' ăn từ đầu tiên!'; cls = ' is-success'; }
        ...
        box.textContent = text; box.className = 'vocab-race-event' + cls; lastResultKey = key;
    }
    ...
    function renderFinish() {
        ...
        ranking.forEach((p,i) => { const el = document.createElement('div'); el.className = 'vocab-race-rank-row rank-' + (i+1) + (me && String(p.user_id) === String(me.id) ? ' is-me' : ''); el.innerHTML = '<span class="vocab-race-rank-no">' + (['🥇','🥈','🥉'][i] || '#'+(i+1)) + '</span><span><strong>' + esc(p.display_name || ('Học viên '+p.slot)) + '</strong><small>' + p.wrong_count + ' lần sai</small></span><b>' + p.score + ' điểm</b>'; host.appendChild(el); });
    ...
    async function steer(delta) {
        const p = myPlayer(); if (!p || isEliminated(p) || claimPending) return;
    ...
    function updateControls() {
        ...
        const lane = p.lane == null ? 2 : Number(p.lane); const eliminated = isEliminated(p);
        ...
        $('vocab-race-action-hint').textContent = eliminated ? 'Bạn bị loại — chờ vòng sau' : claimPending ? '🚀 Đang lao lên...' : obstacleDangerText(lane) || '← Chọn làn → rồi bấm ĂN TỪ';
    }

    async function eatWord() {
        const p = myPlayer(); if (!p || isEliminated(p) || claimPending || room.status !== 'playing') return;
        const lane = p.lane == null ? 2 : Number(p.lane); const roundAtCall = Number(room.round_index);
        claimPending = true; charging = { round: roundAtCall, lane, until: Date.now() + 700 }; updateControls();
        const car = document.querySelector('#vocab-race-track .vocab-race-car.is-me'); if (car) car.classList.add('is-charging');
        await sleep(430);
        ...
        claimPending = false; charging = null; scheduleRefresh();
    }
    ...
    function obstacleLanes(wave) {
        const seed = baseSeed(), round = Number(room && room.round_index || 0);
        const a = (seed + round * 7 + wave * 3) % 5;
        const offset = 1 + ((Math.floor(seed / 5) + round + wave) % 4);
        return [a, (a + offset) % 5];
    }

    function elapsedMs() { const t = Date.parse(room && room.round_started_at || ''); return Number.isFinite(t) ? Math.max(0, Date.now() - t) : 0; }

    function obstacleState() {
        const elapsed = elapsedMs(); let warning = null, falling = null;
    ...
    function updateObstacleVisuals() {
        const track = $('vocab-race-track'); if (!track) return;
        const s = obstacleState();
        track.querySelectorAll('.vocab-race-lane').forEach(el => {
            const lane = Number(el.dataset.lane), warn = el.querySelector('.vocab-race-obstacle-warning'), obs = el.querySelector('.vocab-race-obstacle');
            el.classList.remove('is-obstacle-warning','is-obstacle-active'); warn.classList.remove('is-visible'); obs.classList.remove('is-visible');
            if (s.warning && s.warning.lanes.includes(lane)) { el.classList.add('is-obstacle-warning'); warn.classList.add('is-visible'); const em = warn.querySelector('em'); if (em) em.textContent = (s.warning.remaining/1000).toFixed(1) + 's'; }
            if (s.falling && s.falling.lanes.includes(lane)) { el.classList.add('is-obstacle-active'); obs.classList.add('is-visible'); const travel = Math.max(120, el.clientHeight - 140); obs.style.top = (84 + Math.min(1,s.falling.progress) * travel) + 'px'; }
        });
    }
    ...
    async function resolveObstacleImpacts() {
        ...
        for (let wave = 1; wave <= MAX_OBSTACLE_WAVE; wave++) {
            ...
            if (error && !/already_processed|game_not_playing|stale_round/i.test(String(error.message || ''))) { resolvedObstacleKeys.delete(key); setStatus(translateError(error), 'error'); }
            scheduleRefresh();
        }
    }
    ...
    function tick() {
        ...
        updateObstacleVisuals(); updateControls(); resolveObstacleImpacts();
    ...
    async function resolveTimeout() {
        ...
        timeoutPending = false; if (error && !/game_not_playing/i.test(String(error.message || ''))) setStatus(translateError(error), 'error'); scheduleRefresh();
    }
    ...
    function translateError(err) {
        ...
        const map = { room_not_found:'Không tìm thấy phòng.', room_full:'Phòng đã đủ 4 người.', room_already_started:'Phòng đã bắt đầu.', need_2_to_4_players:'Cần 2–4 học viên.', host_only:'Chỉ chủ phòng được thao tác.', teacher_cannot_play:'Tài khoản giáo viên không tham gia.', stale_round:'Vòng đã chuyển, hãy thử lại.', already_answered:'Bạn đã bị loại hoặc đã hành động vòng này.', round_timeout:'Vòng đã hết thời gian.', lane_changed:'Xe vừa đổi làn, hãy thử lại.' };
